  ...
  return (
    <div className="flex w-full sm:w-[400px] shrink-0 flex-col border-l border-border bg-card h-full relative z-20 shadow-[-10px_0_30px_-15px_rgba(0,0,0,0.1)] transition-transform duration-300">
      ...
      <div className="flex-1 overflow-y-auto">
        {isDetailFetching && selectedId && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-card/80 backdrop-blur-sm">
            <div className="flex flex-col items-center gap-2">
              ...
              <p className="text-xs font-semibold text-indigo-500 uppercase tracking-widest">Enriching Details...</p>
            </div>
          </div>
        )}
        {results.length === 0 && (
        ...
        {/* LIST VIEW */}
        {!selectedLead && results.map((result) => {
          ...
          return (
            <div
              ...
              className={cn(
                ...
                hoveredId === result.external_place_id && "bg-accent/30",
            ...
            >
              <div className="flex gap-3">
                ...
                <div className={cn(
                  ...
                  isDup ? "bg-muted border-border" : "bg-indigo-50 border-indigo-100 text-indigo-600 dark:bg-indigo-500/10 dark:border-indigo-500/20"
                )}>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    {result.rating && (
                      <div className="flex items-center gap-0.5 text-xs font-bold text-amber-500 shrink-0">
                        {result.rating} <Star className="h-3 w-3 fill-amber-500" />
                      </div>
                    )}
                  </div>
          ...
          <div className="p-4 space-y-6 animate-in slide-in-from-right-4 fade-in duration-200">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                {selectedLead.dedup?.is_duplicate ? (
                  <span className="rounded bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-500 border border-emerald-500/20 flex items-center gap-1">
                    <CheckCircle2 className="h-3 w-3" /> Already in Pipeline
                  </span>
                ) : (
                  <span className="rounded bg-indigo-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-indigo-500 border border-indigo-500/20 flex items-center gap-1">
                    <Tag className="h-3 w-3" /> New Discovery
                  </span>
                )}
                {selectedLead.rating && (
                   <span className="rounded bg-amber-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-500 flex items-center gap-1">
                    <Star className="h-3 w-3 fill-amber-500" /> {selectedLead.rating}
                  </span>
                )}
              </div>
              ...
              {selectedLead.business_category && (
                <p className="text-sm text-indigo-400 font-medium mt-1 uppercase tracking-wide text-[10px] bg-indigo-500/10 border border-indigo-500/20 inline-block px-2 py-0.5 rounded">
                  {selectedLead.business_category}
                </p>
              )}
            </div>
            ...
            {!selectedLead.dedup?.is_duplicate && (
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 rounded-lg bg-emerald-600 text-white p-3 text-sm font-semibold shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 transition-colors"
              >
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    {selectedLead.website ? (
                      <a href={selectedLead.website} target="_blank" rel="noreferrer" className="text-sm text-indigo-400 hover:text-indigo-300 underline underline-offset-2 break-all">
                        {selectedLead.website}
                      </a>
                    ) : (
                ...
                {selectedLead.google_maps_url && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <a href={selectedLead.google_maps_url} target="_blank" rel="noreferrer" className="text-sm text-indigo-400 hover:text-indigo-300 underline underline-offset-2">
