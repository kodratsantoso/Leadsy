...
import { AiModeSelector, type AiMode } from "@/modules/ai/components/ai-mode-selector";
  ...
  return (
    <div className="flex w-[340px] shrink-0 flex-col border-r border-border bg-card h-full">
      ...
      <div className="border-b border-border p-4 bg-muted/10">
        ...
        <form onSubmit={handleGeocode} className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="h-8 w-full rounded-md border border-input bg-background pl-8 pr-3 text-xs placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring shadow-sm" 
            />
          </div>
          <button 
            type="submit"
        ...
        {activeArea && (
          <div className="mt-2 text-[10px] text-[var(--status-success)] flex items-center gap-1 font-medium bg-[color-mix(in_oklch,var(--status-success)_10%,transparent)] px-2 py-1 rounded">
            ...
            <div className="space-y-2.5">
              <div>
                <input 
                  ...
                  className="h-8 w-full rounded-md border border-input bg-background px-3 text-xs placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring shadow-sm transition-all" 
                />
              </div>
              ...
              <div className="relative">
                <select
                  ...
                  className="h-8 w-full appearance-none rounded-md border border-input bg-background px-3 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-ring shadow-sm"
                >
          ...
          <div className="space-y-4">
            ...
            <div className="bg-muted/20 border border-border/50 rounded-lg p-3">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs font-bold text-[var(--brand)]">{(radius / 1000).toFixed(1)} km</span>
              </div>
              <input 
      ...
      <div className="border-t border-border p-3 space-y-3 bg-card shadow-[0_-10px_20px_-5px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between gap-2">
          <button 
            ...
            className={cn("flex-1 flex items-center justify-center gap-1.5 rounded-md border py-1.5 text-xs font-medium transition-colors", showFilters ? "border-[color-mix(in_oklch,var(--brand)_30%,transparent)] bg-[color-mix(in_oklch,var(--brand)_10%,transparent)] text-[var(--brand)]" : "border-border text-muted-foreground hover:bg-muted")}
          >
            <SlidersHorizontal className="h-3 w-3" /> Filters
          </button>
          <button 
            onClick={loadHistory} 
            className={cn("flex h-8 w-8 items-center justify-center rounded-md border text-muted-foreground transition-colors", showHistory ? "border-[color-mix(in_oklch,var(--brand)_30%,transparent)] bg-[color-mix(in_oklch,var(--brand)_10%,transparent)] text-[var(--brand)]" : "border-border hover:bg-muted")}
        ...
        <button
          ...
          className="flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-[var(--brand)] to-[oklch(0.558_0.288_302.321)] text-sm font-medium text-white shadow-lg shadow-[color-mix(in_oklch,var(--brand)_25%,transparent)] transition-all hover:shadow-xl hover:-translate-y-0.5 disabled:opacity-50 disabled:shadow-none disabled:transform-none"
        >
