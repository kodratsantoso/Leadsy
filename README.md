...
- Pre-Meeting BANTC Intelligence: Lead detail → Intelligence can generate AI-powered customer intelligence briefs using leads, contacts, WhatsApp context, and Google Grounding. Evaluates BANTC status, generates validation questions, scores meeting readiness, and evaluates product fit.
- AI Usage Tracking: Settings → AI Intelligence → AI Usage tracks real-time token consumption, estimated USD costs, and Google Grounding executions generated by the AI services across the system.
...
- Professional Services Estimator: Professional Services menu provides a full configuration engine (Categories, Templates, Rate Cards, Complexity Matrix) for calculating service implementation ManDays. Lead Detail → Professional Services tab allows seamless creation of implementation estimates connected directly to the sales lead.
