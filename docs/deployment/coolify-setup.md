...
3. Optionally import the committed database snapshot when `IMPORT_LEADSY_DB_SNAPSHOT=true`
...
| `IMPORT_LEADSY_DB_SNAPSHOT` | `true` for first fresh deploy only, then remove/false | No |
| `LARK_OPEN_API_BASE_URLS` | `https://open.larksuite.com/open-apis,https://open.larkoffice.com/open-apis` | No |
| `LARK_ACCOUNTS_BASE_URL` | `https://accounts.larksuite.com` | No |
| `LARK_OAUTH_SCOPES` | `auth:user.id:read` | No |
...
- [ ] If this is a fresh DB that needs current records, set `IMPORT_LEADSY_DB_SNAPSHOT=true` for the first deploy only
- [ ] Lark custom app redirect URL includes `https://leadsy.virtuenet.space/auth/lark/callback`
...
After a successful first snapshot import, set `IMPORT_LEADSY_DB_SNAPSHOT=false` or remove it from Coolify. The migration is recorded as applied, but leaving one-time import flags enabled creates avoidable confusion during rebuilds.
...
- Configure the Lark Custom App credentials from Leadsy `Settings -> Integrations`.
- Add the frontend callback URL in Lark: `https://leadsy.virtuenet.space/auth/lark/callback`.
- Keep Lark SSO enabled in the integration module toggles.
- If a snapshot was restored with a different `APP_KEY`, re-enter the Lark App Secret before using `Test Connection`.
