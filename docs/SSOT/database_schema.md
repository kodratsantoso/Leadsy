...
### Core
- `users` — includes Direct Manager hierarchy and revenue target fields (`direct_manager_id`, `target_period`, `target_revenue`)
...
- `funnel_stages`
- `lead_funnel_history`
- `lead_outcomes` — Closed Won/Lost realization source for dashboard funnels and Achievement Sales; each outcome can reference a `product_id`, `sale_type` (`new_sales` or `upsales`), and its own `deal_size`
- `revenue_rules`
- `lead_revenue_analyses`

### Products & ICP
- `products`
- `product_questions` — one-to-one with products; stores structured JSON question guide for sales discovery (ai_generated, ai_model, updated_by)
...
- `lead_meetings`
- `lead_activities` — timeline records; Meeting activities store evolving BANTC fields (`budget`, `authority`, `needs`, `timeline`, `competitor`)
- `lead_transcripts` — text/manual/file transcript records; can link to `lead_activities.activity_id` and store file metadata for TXT/VTT/SRT/audio/video references
- `lead_ai_evaluations` — AI meeting/transcript evaluation output including summary, sentiment, intent, interest, objections, buying signals, confidence, and next action
- `lead_ai_analyses`
- `lead_follow_ups`
- `lead_bantc_question_guides` — saved per-lead BANTC discovery question guides generated by AI and approved by users
- `sales_visits` — mobile field-sales visit session with lead/user, clock-in/out timestamps, GPS coordinates, distance from lead, risk status/signals, device metadata, visit result, notes, client identity, and signature timestamp
- `sales_visit_media` — photo evidence and signature file records linked to a sales visit
...
- Lead map POIs are served from `leads.lat` and `leads.lng`; marker color is resolved from `funnel_stages.color`.
- Dashboard funnel conversion is cumulative by funnel stage sequence; drilldown uses `funnel_min_sequence` for stage-or-beyond filtering.

### Integration & Multi-Tenant
- `integration_configs`
- `tenants`
- `record_origin_mappings`
- `lark_integrations`, `lark_events`, `lark_syncs`, and `lark_sso_users` — Lark Custom App integration, webhook/event persistence, sync audit, and SSO identity links
- `lark_base_tables` and `lark_base_record_mappings` — Lark Base table configuration and Leadsy-to-Base record identity mappings
...
## Mobile Field Sales Persistence

`sales_visits` and `sales_visit_media` support the Android/iOS field-sales app:

| Table | Purpose |
| --- | --- |
| `sales_visits` | Visit session, clock-in/out coordinates, GPS accuracy, distance from lead, risk status/signals, visit outcome, notes, client identity |
| `sales_visit_media` | Photo evidence and signature files tied to a visit |

Anti fake location is stored as risk evidence, not an absolute guarantee. The backend flags low GPS accuracy, outside-radius visits, mock-location signals, rooted-device signals, and jailbroken-device signals for manager/audit review.

## Lark Base Persistence

`lark_base_tables` stores active Base table sync configuration, while `lark_base_record_mappings` stores the durable relationship between a Leadsy lead and a Lark Base record. This prevents duplicate record creation and allows two-way updates through webhooks or manual push/pull actions.
