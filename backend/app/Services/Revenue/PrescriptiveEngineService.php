...
class PrescriptiveEngineService
{
    ...
    {
        if ($qualified === 'yes' && $score >= 70) return 'within 24 hours';
        if ($score >= 50)                          return 'within 3 days';
        if ($daysSince > 30)                       return 'within 1 week';
    ...
    private function buildReasoning(Lead $lead, float $score, string $qualified, int $daysSince): string
    {
        ...
        if ($lead->industry)     $parts[] = "Industry: {$lead->industry->name}";
        if ($lead->funnelStage)  $parts[] = "Funnel: {$lead->funnelStage->name}";
