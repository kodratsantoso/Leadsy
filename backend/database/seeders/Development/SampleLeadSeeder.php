...
{
    const S_NEW=1;const S_ENRICH=2;const S_QUAL=3;const S_CONTACT=4;
    const S_FOLLOWUP=5;const S_MEETING=6;const S_OPP=7;const S_PROPOSAL=8;
    const S_WON=9;const S_LOST=10;
    const I_MFG=3;const I_RETAIL=4;const I_TECH=5;const I_FIN=6;
    const I_PROP=7;const I_LOG=9;const I_FNB=11;const I_ENRG=12;
    const P_NETSUIT=10;const P_TALENTA=9;const P_QONTAK=15;const P_SIGN=8;
    const P_JEDOX=7;const P_OCEAN=4;const P_QISCUS=5;const P_SEAL=6;
    const P_LARK=11;const P_GWORKSP=24;
    const U_SALES=3;const U_MANAGER=4;const U_GM=5;const U_DIR=6;
    const CH_MAPS=1;const CH_LINKEDIN=12;const CH_SALES=4;
    const CH_REFERRAL=15;const CH_WEBSITE=10;
    ...
    private function seedActivities(int $leadId,array $d,Carbon $now): void
    {
        ...
        if($d['stage']>=self::S_ENRICH){$acts[]=['activity_type'=>'Note','description'=>'Enrichment: profil perusahaan, ukuran bisnis, dan kontak dilengkapi dari sumber publik.','outcome'=>'Profil siap kualifikasi.','activity_date'=>$base->copy()->addDays(3),'user_id'=>$d['owner']];}
        if($d['stage']>=self::S_QUAL){$acts[]=['activity_type'=>'Call','description'=>'Cold call ke '.$c['name'].' ('.$c['title'].'). Memperkenalkan solusi, menggali kebutuhan awal.','outcome'=>'Prospect tertarik. Diminta kirim company profile via email.','activity_date'=>$base->copy()->addDays(5),'user_id'=>$d['owner']];}
        ...
        if($d['stage']>=self::S_MEETING){$acts[]=['activity_type'=>'Meeting','description'=>'Product demo & discovery session bersama '.$c['name'].' dan tim terkait.','outcome'=>'Demo sukses. Prospect sangat tertarik fitur '.$this->keyFeature($d['product']).'. Diminta proposal teknis dan komersial.','activity_date'=>$base->copy()->addDays(21),'user_id'=>$d['owner']];}
